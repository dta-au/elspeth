...
def test_checked_golden_matches_live_mechanical_projection_without_writing() -> None:
    ...
    assert "54 plugins" in result.stdout
...
def test_checked_golden_has_exact_counts_hashes_and_reviewed_fields() -> None:
    ...
    assert [sum(entry["kind"] == kind for entry in plugins) for kind in ("source", "transform", "sink")] == [9, 36, 9]
...
def test_production_config_validation_constructs_exactly_the_75_reviewed_subjects() -> None:
    matrix = json.loads(GOLDEN.read_text(encoding="utf-8"))
    ...
    assert sum(variant == "default" for _, variant in validated) == 44
