...
class TestOpenRouterConfig:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "http://192.168.1.10:8199/v1",
            "http://user:pass@127.0.0.1:8199/v1",
...
class TestOpenRouterTemplateFeatures:
    ...
    def test_lookup_data_accessible_in_template(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="Classify as {{ lookup.categories[0] }} or {{ lookup.categories[1] }}: {{ row.text }}",
    ...
    def test_two_dimensional_lookup_row_plus_lookup(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="Use tone: {{ lookup.tones[row.tone_id] }}. Message: {{ row.text }}",
    ...
    def test_lookup_hash_included_in_output(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="Categories: {{ lookup.cats }}. Input: {{ row.text }}",
                lookup={"cats": ["A", "B", "C"]},
            )
        )
        init_ctx = make_context(landscape=audit_writer)
    ...
    def test_all_audit_fields_present_with_lookup(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="{{ lookup.prompt_prefix }} {{ row.text }}",
                prompt_template_source="prompts/prefixed.j2",
    ...
    def test_no_lookup_has_none_hash_in_output(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="Simple: {{ row.text }}",
    ...
    def test_lookup_iteration_in_template(self, audit_writer: InMemoryAuditWriter, collector: CollectorOutputPort, chaosllm_server) -> None:
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="""Classify into one of:
...
Text: {{ row.text }}""",
    ...
    def test_template_error_includes_source_in_error_details(
        ...
        transform = LLMTransform(
            _openrouter_config(
                model="openai/gpt-4",
                prompt_template="Missing: {{ row.required_field }}",
