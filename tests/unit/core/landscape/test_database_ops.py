...
Every audit write must succeed. execute_insert_on raises
AuditIntegrityError when rowcount == 0, which is the enforcement mechanism
for that invariant. These tests verify caller-owned statements against a real
...
def ldb() -> LandscapeDB:
    ...
    sa.Table("test_rows", metadata, sa.Column("id", sa.String, primary_key=True), sa.Column("value", sa.String))
...
class TestExecuteFetchone:
    ...
    def test_returns_row_when_found(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="r1", value="hello"))
    ...
    def test_raises_on_multiple_rows(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="a", value="first"))
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="b", value="second"))
...
class TestExecuteFetchall:
    ...
    def test_returns_all_rows(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="x", value="one"))
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="y", value="two"))
    ...
    def test_rejects_write_statement_even_with_returning(self, tmp_path: Path) -> None:
        ...
        metadata = sa.MetaData()
        table = sa.Table("test_rows", metadata, sa.Column("id", sa.String, primary_key=True), sa.Column("value", sa.String))
        metadata.create_all(db.engine)
        ops = DatabaseOps(db)
        with db.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, table.insert().values(id="a", value="before"))
    ...
    def test_fetchall_many_read_only_handle_uses_one_sqlite_snapshot(self, tmp_path: Path) -> None:
        ...
        def _mutate_after_first_select(conn, cursor, statement, parameters, context, executemany):
        ...
        try:
            rows_by_query = ReadOnlyDatabaseOps(read_only).execute_fetchall_many(
                [table.select().where(table.c.event_id == "first"), table.select().where(table.c.event_id == "second")]
            )
        finally:
...
class TestDatabaseOpsErrorScrubbing:
    ...
    def test_insert_error_message_omits_sqlalchemy_statement_and_bound_values(self, ops: DatabaseOps, ldb: LandscapeDB) -> None:
        with pytest.raises(AuditIntegrityError) as exc_info, ldb.write_connection() as write_conn:
            ops.execute_insert_on(
                write_conn,
...
class TestExecuteInsert:
    ...
    def test_insert_succeeds(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="new", value="data"))
        row = ops.execute_fetchone(test_table.select().where(test_table.c.id == "new"))
        assert row is not None
        assert row.value == "data"

    def test_insert_zero_rows_raises(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        """INSERT OR IGNORE on a duplicate PK executes without SQL error but returns rowcount=0."""
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="row1", value="first"))
        with pytest.raises(AuditIntegrityError, match="zero rows affected"), ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, sa.text("INSERT OR IGNORE INTO test_rows (id, value) VALUES ('row1', 'dup')"))

    def test_insert_error_message_contains_operation_name(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="row2", value="original"))
        with pytest.raises(AuditIntegrityError) as exc_info, ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, sa.text("INSERT OR IGNORE INTO test_rows (id, value) VALUES ('row2', 'dup')"))
        assert "execute_insert" in str(exc_info.value)

    def test_insert_context_in_error(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="row3", value="original"))
        with pytest.raises(AuditIntegrityError, match="insert context label"), ldb.write_connection() as write_conn:
            ops.execute_insert_on(
                write_conn,
    ...
    def test_insert_no_context_omits_parens(self, ops: DatabaseOps, test_table: sa.Table, ldb: LandscapeDB) -> None:
        with ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, test_table.insert().values(id="row4", value="original"))
        with pytest.raises(AuditIntegrityError) as exc_info, ldb.write_connection() as write_conn:
            ops.execute_insert_on(write_conn, sa.text("INSERT OR IGNORE INTO test_rows (id, value) VALUES ('row4', 'dup')"))
