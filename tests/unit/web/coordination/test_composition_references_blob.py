...
reverting ``_names_blob`` to an exact comparison must turn them red again.
"""
...
from elspeth.web.coordination.repository import _composition_references_blob, _names_blob
...
def test_a_non_string_marker_value_is_not_a_match() -> None:
    assert _names_blob(None, _BLOB_ID) is False
    assert _names_blob(1234, _BLOB_ID) is False
    assert _names_blob(["0f1e2d3c-4b5a-6978-8a9b-0c1d2e3f4a5b"], _BLOB_ID) is False
