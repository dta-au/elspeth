...
def test_blob_inline_resolutions_schema_epoch_is_45(engine) -> None:
    # 44: guided_operations.failure_code gains planner_repair_exhausted (elspeth-5904b1683a).
    assert SESSION_SCHEMA_EPOCH == 45
    with engine.connect() as conn:
        assert conn.execute(text("PRAGMA user_version")).scalar_one() == 45
