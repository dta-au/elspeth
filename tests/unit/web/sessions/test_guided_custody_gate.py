...
def test_every_composition_states_insert_site_is_preceded_by_the_custody_gate() -> None:
    ...
    ``insert(composition_states_table)`` in the sessions service sits inside a
    method that calls ``assert_guided_custody_persistable`` before the insert.
    A new insert site must either route through ``_insert_composition_state``
    or carry its own gate call."""
    source_lines = Path(service_module.__file__).read_text(encoding="utf-8").splitlines()
    def_pattern = re.compile(r"^    (async )?def (\w+)\(")
    sites: dict[str, int] = {}
    for index, line in enumerate(source_lines):
        if "insert(composition_states_table)" not in line:
            continue
        owner_line = next(i for i in range(index, -1, -1) if def_pattern.match(source_lines[i]))
        owner = def_pattern.match(source_lines[owner_line]).group(2)  # type: ignore[union-attr]
        body = "\n".join(source_lines[owner_line:index])
        assert "assert_guided_custody_persistable(" in body, f"{owner} inserts a composition_states row without the custody gate"
        sites[owner] = index + 1
    assert set(sites) == {"_insert_composition_state", "save_composition_state", "set_active_state"}, sites
