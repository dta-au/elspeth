...
def test_initialize_session_schema_rejects_epoch_35_database() -> None:
    ...
    with pytest.raises(
        SessionSchemaError,
        match=r"Session DB schema version 35 does not match SESSION_SCHEMA_EPOCH=39.*Delete the session DB file and restart",
    ):
        initialize_session_schema(eng)
...
def test_epoch_36_database_without_declined_result_contract_fails_at_sentinel(tmp_path) -> None:
    ...
    with pytest.raises(
        SessionSchemaError,
        match=r"Session DB schema version 36 does not match SESSION_SCHEMA_EPOCH=39.*"
...
def test_epoch_30_database_without_schema_9_operation_contract_fails_closed_with_recreate_guidance(tmp_path) -> None:
    ...
    with pytest.raises(
        SessionSchemaError,
        match=r"Session DB schema version 30 does not match SESSION_SCHEMA_EPOCH=39.*"
        r"Delete the session DB file and restart",
    ):
        initialize_session_schema(stale_engine)
