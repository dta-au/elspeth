...
def test_current_cutover_requires_live_epoch_contract_and_forbids_downgrade_repair() -> None:
    ...
    assert "0.8.0 blob cleanup, guided decline, and aggregation recovery" in current_cutover
    assert f"session epoch {SESSION_SCHEMA_EPOCH}" in current_cutover
    assert "Landscape epoch 36" in current_cutover
    assert f"0.8.0 advances `SESSION_SCHEMA_EPOCH` from 35 to {SESSION_SCHEMA_EPOCH}" in current_cutover
