...
def test_guided_confirmation_admission_hard_cut_runs_inside_session_epoch_37() -> None:
...
def test_epoch_35_session_store_fails_before_schema_use(tmp_path: Path) -> None:
    ...
    with pytest.raises(SessionSchemaError, match=r"SESSION_SCHEMA_EPOCH=37.*Delete the session DB file and restart"):
        initialize_session_schema(engine)
