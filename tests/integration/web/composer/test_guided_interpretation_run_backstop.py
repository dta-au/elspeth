...
def _build_execution_service(
    ...
    execute() returns a real run_id. The service is bound to pytest's running
    loop so its transferred execution lease can settle through the real
    completion callback; `_run_pipeline` is stubbed, so the background bridge
    is never otherwise exercised.
...
async def test_unresolved_card_blocks_run_resolving_permits(
    ...
    try:
        ...
        blocked_lease = await SessionOperationLease.acquire(
