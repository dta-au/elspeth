...
_RELEASE_LANE_TO_SELECTOR = {
    "local": "postgresql-16-aws",
    "live-aws": "protected-live-aws",
    "live-azure": "protected-live-azure",
    "live-dataverse": "protected-live-dataverse",
    "live-chroma": "protected-live-chroma",
    "live-provider": "protected-live-provider",
}
...
def _catalog_cells(catalog: Mapping[str, Any]) -> tuple[set[Cell], set[tuple[str, str]], dict[tuple[str, str], str | None]]:
    ...
    return required_cells, pb09_variants, plugin_by_subject


def _target_lane(cell: Cell, plugin_by_subject: Mapping[tuple[str, str], str | None], release_lanes: Mapping[str, str]) -> str:
    ...
    if leg_id != "PB-09" or plugin_key is None:
        return "postgresql-16-aws"
    release_lane = release_lanes[plugin_key]
    return _RELEASE_LANE_TO_SELECTOR[release_lane]
...
def validate_selector_manifest_data(
    ...
    *,
    ...
    _require(
        type(manifest.get("schema_version")) is int and manifest.get("schema_version") == 1, "selector manifest schema_version must be 1"
    )
    ...
    required_cells, catalog_variants, plugin_by_subject = _catalog_cells(catalog)
    ...
    _require(
        set(plugin_release_lanes.values()) <= set(_RELEASE_LANE_TO_SELECTOR),
    ...
    _require(lane_ids == list(LANE_SPECS), "selector manifest lanes do not match the closed lane inventory")
    ...
    for raw_lane in raw_lanes:
        ...
        for raw_cell in raw_cells:
            ...
            expected_lane = _target_lane(parsed, plugin_by_subject, plugin_release_lanes)
