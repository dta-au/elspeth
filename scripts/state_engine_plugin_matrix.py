...
def _mechanical_projection() -> list[dict[str, object]]:
    ...
    if len(rows) != sum(EXPECTED_COUNTS.values()):
        raise ValueError(f"expected {sum(EXPECTED_COUNTS.values())} plugins, discovered {len(rows)}")
    return rows
...
def _live_plugin_types_by_key() -> dict[str, PluginType]:
    ...
    if len(plugin_types) != sum(EXPECTED_COUNTS.values()):
        raise ValueError(f"expected {sum(EXPECTED_COUNTS.values())} plugins, discovered {len(plugin_types)}")
    return plugin_types
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "check":
            check(args.golden)
            print(f"state-engine plugin matrix: valid ({sum(EXPECTED_COUNTS.values())} plugins, {EXPECTED_VARIANT_COUNT} PB-09 variants)")
            return 0
