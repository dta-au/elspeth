...
def source_snapshot_sha256(
    *,
    ...
    records = [
        {
            "path": item.label,
            "sha256": hashlib.sha256(item.physical_path.read_bytes()).hexdigest(),
        }
        for item in inputs
    ]
...
def capture_source_snapshot(
    *,
    ...
    logical_allowlist_dir = Path(logical_allowlist_dir or allowlist_dir).resolve()
    first = _observe_source_snapshot(
        source_root=source_root,
        allowlist_dir=allowlist_dir,
        logical_allowlist_dir=logical_allowlist_dir,
    )
    second = _observe_source_snapshot(
        source_root=source_root,
        allowlist_dir=allowlist_dir,
        logical_allowlist_dir=logical_allowlist_dir,
    )
...
def _observe_source_snapshot(
    *,
    ...
    return SourceSnapshot(
        ...
        source_snapshot_sha256=hashlib.sha256(canonical.encode("utf-8")).hexdigest(),
    )
...
def _require_tracked_inputs(*, repo_root: Path, inputs: tuple[_SnapshotInput, ...]) -> None:
    ...
    for item in inputs:
        logical_path = item.logical_path.resolve()
