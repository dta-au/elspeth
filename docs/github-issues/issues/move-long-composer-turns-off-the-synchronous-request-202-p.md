...
A composer turn can run for minutes inside one buffered HTTP request that puts zero bytes on
the wire until it finishes. Any intermediary that closes idle connections — a CDN edge, an
inspecting proxy, a load balancer — cuts it first, and the user sees a generic send failure
for work the server may have completed.
...
The timeout belongs to a hop the deployment cannot always enumerate — a proxy on the
client's own machine is invisible to the server — so no declared request ceiling can be
guaranteed correct.

## Where the work starts

The HTTP surface is `src/elspeth/web/sessions/routes/`; the progress contract the browser
polls is `src/elspeth/web/composer/progress.py`; the browser store is
`src/elspeth/web/frontend/src/stores/sessionStore.ts`. Three routes share the buffered
long-turn shape and the `_track_compose_inflight` dependency, so a fix covering one will be
re-argued twice:
...
Tried and abandoned; recorded so it is not rebuilt. The prototype relocated the handler body
into a child task and streamed whitespace heartbeats from the parent, carrying the outcome in
a deferred body envelope. It broke five tests under `tests/unit/web/sessions/`, structurally
...
`8f4384936`, most machinery already exists:
...
- The browser already polls twice per turn — `startComposerProgressPolling` and
  ...
  `loadInflightMessages` (`:2653`) already merges server-authoritative messages with
  optimistic local ones mid-turn.
- The progress contract already carries a richer failure taxonomy than the HTTP status:
  `convergence_progress_event` (`progress.py:339`) publishes `phase="failed"` with a `reason`
  naming which budget was exhausted, plus `headline`, `evidence` and `likely_next`;
...
- `ComposerRequestLease` (`progress.py:48-54`) and `renew_request` (`progress.py:106`) exist,
  as does durable multi-instance progress authority.
...
2. **Reason coverage.** Convergence failures and client cancellation have progress reasons
   today. Whether `llm_unavailable`, `llm_auth_error`, `audit_integrity_error` and
   `policy_blocked` do must be measured, not assumed.
...
- A turn that outlasts an idle timeout still reports its real outcome. The test closes the
  connection deliberately rather than waiting on a real network.
- Every one of the 16 `raise HTTPException` sites in `messages.py` maps to a pre-202 status or
  a named terminal progress reason, pinned by a test so a new raise site cannot be added
  without a mapping.
...
Large, and design-first — do not start with code. Item 2 is a good self-contained first task:
enumerate the raise sites in `messages.py`, map each to a progress reason or an admission-time
status, and publish the table. That answer is needed before the rest can be designed.
