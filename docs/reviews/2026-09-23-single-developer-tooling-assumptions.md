...
**Source:** memory archived tool reference.

> **Documented in [the repository's] `CLAUDE.md`** under the post-managed-block
> section "legacy issue tracker from a worktree" (the upstream `<!-- legacy-tracker:instructions -->`
> block is regenerated by legacy issue tracker, so project addenda must live outside it).
...
`retired-code-index` hook carries no timeout at all. This is the maintainer's toolchain
wired into the shared configuration — precisely what ADR-043 split apart, leaking
back in through `.claude/settings.json`.

**Recommendation — MUST CHANGE.** Either guard each hook (`command -v legacy issue tracker
>/dev/null 2>&1 && legacy issue tracker session-context`), or move the tool hooks into the
...
> `legacy-tracker` tracks tasks for this project. Data lives in archived tool reference.

**Measured today:** archived tool reference does not exist. The live database is under
archived tool reference — a directory the repository's own `.gitignore` labels:

> `# Retired tooling residue (ADR-043).`
> `.weft/`

The memory store already recorded this (archived tool reference:
"the live DB is archived tool reference (the installer-written toolchain.md
block saying archived tool reference is wrong)").
...
`explore-and-pin` and `retired-code-index-workflow` are real directories, the latter
...
**Source:** tracked `AGENTS.md` § legacy issue tracker Issue Tracker, which states:
...
```
AGENTS.md:373:<!-- legacy-tracker:instructions:v3.1.0:c1c023c3 -->
AGENTS.md:374:<!-- legacy-tracker:last-writer:legacy-tracker install -->
AGENTS.md:394:<!-- retired-code-index:instructions:v1.6.0:39edbf6d -->
```

`docs/maintainer/toolchain.md` confirms the mechanism and says so plainly:

> The legacy issue tracker and retired code index blocks below are installer-written mirrors: their
...
The two copies have already drifted: the legacy issue tracker block is content hash
`c1c023c3` in `AGENTS.md` but `65e6fb25` in `toolchain.md`, and retired code index is
...
will be silently reverted the next time anyone runs archived tool command; second,
...
**Source:** archived tool reference:

> When multiple agents call archived tool command
> simultaneously, both think they own the issue. legacy issue tracker 2.0 solves this with
...
| legacy issue tracker primitive | Under GitHub Issues |
|---|---|
...
> cleared, so archived tool command / `work_start_next` skip it forever.
...
**Measured:** tracked files referencing legacy issue tracker include
...
archived tool reference reference files.

The `bug-sweep` skill's own description is explicit:

> have them lodge verified findings in legacy issue tracker under one sweep tag, then
...
semantics that is properly written down. But it is expressed in legacy issue tracker label
syntax and cites legacy issue tracker CLI verbs for keeping it current, so it needs porting
...
> (46 issues, ~121 archived tool command commands) was denied by the
...
> legacy issue tracker's CLI calls `realpath()` on the DB resolved from archived tool reference and
...
7. **D2** — make `AGENTS.md`/`CLAUDE.md` tracker-neutral; move legacy issue tracker guidance
...
- Memory store, 32 files read in full: the branch/merge/push set
  ...
  the tracker set (`legacy-tracker_no_deferred_status`, `legacy-tracker_ready_means_unassigned`,
  `legacy-tracker_mcp_vs_cli_actor`, `legacy-tracker_cli_in_worktrees`, `verifying_is_not_closed`,
...
- `.agents/skills/lane-manager/SKILL.md` (first 70 lines),
  ...
  archived tool reference (first 40),
  archived tool reference (first 60),
...
- archived tool command — archived tool reference absent, archived tool reference present.
- `ls tools/push-slot.sh` — absent.
- `grep -n -i worktree CLAUDE.md` + `wc -l CLAUDE.md` — 23 lines, one worktree
  mention, no "legacy issue tracker from a worktree" section.
