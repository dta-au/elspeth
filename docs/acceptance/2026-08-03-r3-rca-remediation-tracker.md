...
| `/root/fix_grounded_option_constraints` | Ground exact safe option obligations, keep private values out of provider context, and reject contradictory constraints | `.claude/worktrees/grounded-option-constraints`; `codex/fix-grounded-option-constraints` | Deferred-intent authority/admission/planning seams and direct tests | None | Interim `5511eb5f5` is explicitly non-integrable; final amendment must revalidate the deterministic custody-safe proposal against exact prepared authority before terminal settlement and before blob publication, then close alias/cardinality/schema-boundary regressions and pass fresh localized/security review |
