...
Last refreshed: 2026-08-04T08:07:06+10:00 (Australia/Canberra)
Filigree snapshot: 2026-08-04T08:07:06+10:00
...
| `/root/fix_composer_provider_telemetry` | Project durable Composer provider-call audit facts into bounded operator metrics only after commit | `.claude/worktrees/composer-provider-telemetry`; `codex/fix-composer-provider-telemetry` | New provider projector, session post-commit seam, request/operator telemetry, and direct tests; grounded Composer files excluded | None | Candidate `98c0ff961` held after review proved cancellation can commit audit but skip projection; deterministic drain/project/re-raise repair and re-review active |
| `/root/fix_guided_respond_request_telemetry` | Mount provider-capable guided/respond on the shared request telemetry lifecycle | Isolated worktree/branch from release with `98c0ff961` available as a temporary combined test base | `guided.py` route-signature dependency hunk only and direct request-telemetry tests | None | Active strict TDD for `4d0d239886`; shares `guided.py` with grounded work only at disjoint hunks and does not touch its planner-intent construction or telemetry production files |
