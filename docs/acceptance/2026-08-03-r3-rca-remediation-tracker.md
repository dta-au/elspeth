...
| `elspeth-5904b1683a` | P1 | `fixing` | `claude-f14-fix` | Live acceptance 2026-08-04 measured 4/5 canonical replan failures (plain guided 3/3) — worse than the briefed ~2/10. RCA settled (comment 2285): regression from `8ba6f97a8` — the candidate-global `withhold_candidate_facts` predicate blinds guided repair feedback (guided finalization always mutates the candidate), so exhaustion is deterministic given any invalid first candidate and surfaces as a 502 misattributed to the provider. Four-part entry-scoped fix specified in `docs/acceptance/2026-08-04-f14-and-battery-handover.md`; fix worktree `f14-repair-feedback` holds interrupted uncommitted partial work (one file) — restart from spec recommended |
