...
| Controlled counterpart | **None existed at issue time.** If a controlled issue is produced later, it is authoritative for protected resource detail |
...
| Risk and assurance | **Red** | Release readiness depends on an open operator-only assurance action followed by independent verification. That action is outside agent authority. Source: [D-001](raid-register.md#live-register), Filigree records `elspeth-97d0c15eb6` and `elspeth-618b5100b8`, captured `2026-08-29T18:14:21+10:00` to `18:14:24+10:00`; this does not claim a fresh gate run |
| Overall | **Red** | ADR-024 requires the worst load-bearing component to govern the overall status; the release-assurance dependency above is Red. Source: ADR-024 and D-001, as of this report |
...
action, post-operator verification, and release preparation. The first step is
in progress; the handoff and dependent operator and verification steps remain
open. This is progress toward release assurance, but it is not evidence that
release gates are currently complete. Source: Filigree critical path and
selected records at the cut-offs above.
...
| [MF-003](milestone-forecast-register.md#current-milestones) | Verify operator outputs and complete release preparation | Blocked by MF-002 | Commitment **Unknown**; forecast **not established** |
