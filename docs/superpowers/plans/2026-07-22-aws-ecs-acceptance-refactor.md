...
Use quiet pytest output for both repository lanes. This is not reduced coverage: a sink-effect admission-order test deliberately replaces `os.environ` with a fail-closed mapping, and verbose xdist progress can concurrently ask that mapping for terminal-width variables and crash the test runner rather than the product code. Leave the coverage lane serial, matching CI and ELSPETH's pytest plugin: explicit xdist changes the full-lane coverage result and can print a pytest-cov threshold failure without propagating a failing controller exit status.
...
Keep the same quiet-output and serial-coverage requirements as Task 0 Step 5.
