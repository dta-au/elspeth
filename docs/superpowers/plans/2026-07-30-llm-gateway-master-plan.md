...
| 4 | Terraform shared-module generalization (`deployment lifecycle` × `llm backend` axes) + Scenario C — unchanged in shape by the Phase 3 rescope: the gateway sidecar is still a sidecar; ELSPETH points at it by base URL | written at Phase 3 close | Scenario A/B contract tests unchanged-green; Scenario C plans as `first + custom_gateway` |
