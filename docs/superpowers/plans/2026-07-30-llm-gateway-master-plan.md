...
| 3 | **(rescoped 2026-07-31)** Endpoint affordance: gateway contract header made optional (plain OpenAI clients accepted); per-role Composer endpoint settings (`composer_endpoint_base_url`/`composer_advisor_endpoint_base_url` + fail-closed credential pairing); Composer tool round-trip proven against the real gateway; risk-transfer docs. The `ComposerCompletionClient` boundary and nine-caller migration are cancelled — see `2026-07-31-llm-gateway-phase3-endpoint-affordance.md`. Amendment 2 discharged (bedrock.py keeps `_litellm_acompletion` as a documented exemption); amendment 3 restated as vacuous until the pipeline grows a seed concept | `2026-07-31-llm-gateway-phase3-endpoint-affordance.md` (supersedes the deleted `2026-07-30-llm-gateway-phase3-composer.md`) | DELIVERED, merged to release/0.7.2 (`6814dc9f2`, gated at `0addb019d`: 34588 passed / 0 failed; `elspeth-lints check` exit 0) |
| 4 | Terraform shared-module generalization (`deployment lifecycle` × `llm backend` axes) + Scenario C — unchanged in shape by the Phase 3 rescope: the gateway sidecar is still a sidecar; ELSPETH points at it by base URL | `2026-08-09-llm-gateway-phase4-terraform-scenario-c.md` (written 2026-08-09; the "at Phase 3 close" obligation was discharged late) | Agent half DELIVERED 2026-08-09 (051c81eef…d81aa44de): axes explicit, Bedrock surfaces backend-conditional (fail-closed control posture), gateway sidecar + admission + secrets separation, scenario-c root + contract tftests (4/4; A 20/20 unmodified), doctor sidecar parity, C plan-approval admission. LIVE half operator-gated: deploy, health verification, disposable acceptance, C evidence schema |
| 5 | Verification closeout: full suite, `elspeth-lints check`, local reference acceptance, evidence. NOTE: the `--no-ff` merge to release/0.7.2 already happened at Phase 3 close (2026-07-31, `6814dc9f2` + `0addb019d`, gated on the merged result) — Phase 5 is now verification/evidence reconciliation over the merged tree plus Phase 4's Terraform delivery | checklist in this doc | All green |
