...
- **WS1a Task 8a: RETIRED (maintainer ruling 2026-08-22 — nested is NEW behaviour).** No nested scenario dirs exist; §4.1a rows 2–4 have NO differential corpus coverage and are pinned instead by this plan's pure-path truth tables (Task 2) and mint-integration tests, with new corpus scenarios owed post-rewrite.
...
- **AMENDED by maintainer ruling 2026-08-22 (John): nested regions are NEW behaviour.** WS1a Task 8a was retired — today's engine REJECTS both nested topologies at build time (GraphValidationError), so no pre-flip differential oracle for §4.1a rows 2–4 can exist. The freeze covers the 15 depth-1 scenarios only; nested semantics get pinned by NEW (non-differential) corpus scenarios once this rewrite makes the topologies buildable (partial wiring preserved in `.superpowers/sdd/2026-08-21-unified-lineage-ws1a-model-core/task-8a-*`). Protocols §S1 executed before the WS1b flip. **Oracle frozen at e77fbfe2ba187cefd98a0bde08b299f41b43a7ea; freeze commit a74128d93 (49 depth-1 snapshots, write→compare round-trip verified).**
...
**Entry criteria (amended per the 2026-08-22 nested-is-new-behaviour ruling):** the protocols plan's Task 3 freeze commit exists on the branch (a74128d93 — 49 depth-1 snapshots under `tests/fixtures/dag_scenario_corpus/oracle_freeze/v1/`, recorded at pre-flip HEAD e77fbfe2b, `tests/integration/core/dag/test_oracle_freeze.py` green in compare mode; NO nested snapshots — Task 8a retired, nested topologies are unbuildable pre-rewrite); WS1a is fully landed and green (Tasks 1–11; 8a retired by ruling); Phase A is fully committed. Record `git rev-parse HEAD` — this is the flip's base.
...
- [ ] **Step 2: Frozen-oracle diff.** Run the protocols plan's compare gate over every corpus scenario against the stored pre-flip snapshots: `pytest tests/integration/core/dag/test_oracle_freeze.py -q` (compare mode — `ELSPETH_ORACLE_FREEZE` unset). Acceptance, verbatim from spec §11:
  - FROZEN fixtures: all four stable projection classes byte-identical. This covers the 15 depth-1 scenarios in freeze commit a74128d93 (NO nested scenarios — Task 8a retired by the 2026-08-22 ruling), including the r23 casualty `parallel-coalesces` (stays frozen THROUGH WS1, leaves the set only at WS2) and `fork-multiple-terminals-partial-failure` — pure fan-out, LEGAL under §7 rule 2 (2026-08-22 spec correction), permanently FROZEN, not a casualty.
