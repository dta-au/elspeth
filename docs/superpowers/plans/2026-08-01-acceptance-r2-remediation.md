...
> **AMENDED 2026-08-01 (execution-time adjudication).** Steps 1–3's blocking
> `gate_condition_constant`/`gate_condition_dead` codes are SUPERSEDED: the
> constant-condition fork (`condition: "True"`, both routes forking) is
> ELSPETH's documented fan-out idiom — prescribed by the composer's own Build
> Macro (`skills/pipeline_composer.md:262`), generated by our scaffolds
> (`tools/_common.py:689`, `planner_authoring_aids.py:1094,1187`), canonical in
> 14+ examples and the DAG scenario corpus, and pinned by the authoring-surface
> parity property test. Blocking it would wedge our own scaffolds (the failure
> class Task 1 fixed). Replacement, preserving the task's intent (a stated
> threshold must never silently become a double-write): (1) a planner-side
> REPAIRABLE fidelity guard `gate_condition_ignores_stated_threshold` — fires
> only when the driving instruction contains comparison/threshold language AND
> the candidate carries a constant-condition gate, feedback quoting the
> detected comparison; (2) a NON-BLOCKING fan-out advisory (mirroring
> `identity_node_advisory`) for constant-condition gates whose boolean routes
> resolve to identical destination sets with undifferentiated branches; (3) no
> review-card change (display verified faithful). Half (b) — coverage credit —
> is unchanged. The model-layer complement is Task 9's advisor rubric.
