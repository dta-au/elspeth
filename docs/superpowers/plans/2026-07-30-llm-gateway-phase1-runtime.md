...
- Every commit: stage by pathspec, message style `feat(gateway): …` / `test(gateway): …`; run the task's tests before committing.
...
- Produces:
  ...
  - `ErrorClassification(code: str, retryable: bool)` — `code` must be one of the closed gateway error-code strings (import the vocabulary as a `frozenset[str]` constant `CLASSIFIABLE_CODES` = `{"context_length_exceeded","content_policy_rejected","upstream_rate_limited","upstream_timeout","upstream_unavailable","upstream_response_invalid"}`).
...
- Produces (pydantic, `extra="forbid"` everywhere):
  ...
  - `def bounds_check(request: ChatRequest, bounds: Bounds) -> None` raising `GatewayError(invalid_request)`; `Bounds` model defined in Task 6 — for this task declare `Bounds(BaseModel)` HERE with fields `max_messages: int, max_tools: int, max_string_chars: int, max_schema_bytes: int, max_schema_depth: int` (Task 6 imports it).
...
- Produces:
  ...
  - `async def invoke(self, plan: InvokePlan) -> UpstreamResult`:
    ...
    5. `httpx.TimeoutException` → `UPSTREAM_TIMEOUT`; `httpx.TransportError` → `UPSTREAM_UNAVAILABLE`; 429 → `UPSTREAM_RATE_LIMITED`. NO retry/replay for any of these (respx call_count == 1).
    6. Other statuses (2xx, 4xx, 5xx) → returned as `UpstreamResult` for adapter classification.
...
- Produces:
  ...
  - `app.py`: `def create_app(config: GatewayConfig, *, adapter: AdapterProtocol | None = None, upstream_client: httpx.AsyncClient | None = None) -> FastAPI`:
    ...
    - `GET /readyz` → `200/503` JSON exactly: `{"ready": bool, "contract_major": 1, "adapter": {"name","version","adapter_api_major","fingerprint"}, "capabilities": [...], "model_aliases": [...], "mapping_generation": "...", "errors": ["safe_code", ...]}` — validates config presence, adapter import + `validate_configuration({})`, non-empty mappings; NO OAuth call, NO upstream call.
...
- Produces:
  ...
  - `mock/upstream.py`: `def create_mock_upstream_app(*, require_bearer_prefix: str = "mock-token-") -> FastAPI` — `POST /v1/invoke` implementing the fictional schema deterministically: last `conversation` entry's text `"USE_TOOL <op> <json>"` → invocation response with halt `operations`; request `format.kind == "schema"` → body echoing `{"echo": "<last user text>"}` serialized per schema's first property name; otherwise text response `"MOCK:" + <last user text>`; accounting = `{"input_units": total chars // 4, "output_units": reply chars // 4}`; missing/foreign bearer → 401; text `"TRIGGER_FAULT <kind>"` → `{"fault": {"kind": "<kind>"}}` with status 429 for throttle, 503 overloaded, 400 screening/too_long.
  - `mock/stack.py`: `async def start_local_stack(gateway_port=8787, oauth_port=8788, upstream_port=8789)` and `python -m mock.stack` entry — runs all three uvicorn servers with a fully wired env (documented mock secrets), prints the curl example from the README.
...
- Produces:
  - `gateway/conformance/conftest.py`: fixture `gateway_client` — if env `GATEWAY_CONFORMANCE_URL` is set, a real `httpx.AsyncClient(base_url=...)` (image-qualification mode); otherwise in-process: build mock oauth + mock upstream apps, mount them via `httpx.MockTransport`-style ASGI dispatch into the gateway's upstream client, `create_app(...)` with reference adapter, wrap in `ASGITransport`. Also fixtures `bearer` and `declared_capabilities` (parsed from `/readyz`).
  - Conformance tests keyed by declared capabilities: every test first reads `/readyz`; tests for undeclared capabilities assert the gateway REJECTS the feature with `capability_unsupported` (claiming-and-failing is the only failure). Coverage: contract header both ways incl. error responses; auth strictness; unknown-field rejection; text completion happy path; `json_object` and `json_schema` modes; tool call + tool result round trip; error envelope shape for `model_not_allowed`, `capability_unsupported`, `invalid_request`; `/readyz` identity fields present and stable across two calls.
...
```
  ...
  - Root `tests/gateway_runtime/test_inprocess_conformance.py`: runs the full conformance suite in-process via `pytest.main([str(_REPO_ROOT / "gateway" / "conformance"), "-q"])` in a subprocess (`sys.executable -m pytest`) and asserts exit code 0 — so the CI-equivalent `pytest tests/` gate covers the gateway without importing it into the ELSPETH test session.
...
**Interfaces:**
- Produces: multi-stage Dockerfile — builder installs `gateway/` wheel into a venv; final stage `python:3.12-slim`, copies venv only, `USER 65532:65532`, `ENV PYTHONDONTWRITEBYTECODE=1`, no shell entrypoint (`ENTRYPOINT ["python", "-m", "uvicorn", "elspeth_llm_gateway.core.app:build", "--host", "0.0.0.0", "--port", "8787", "--factory"]` with `build()` = `create_app(load_config(os.environ))`), `EXPOSE 8787`, read-only-rootfs compatible (no writes outside `/tmp`). README: quick-start (mock stack), onboarding path 1–6 from the design, derived-image instructions (`FROM elspeth-llm-gateway:<digest>` + `pip install yourorg-adapter==X.Y.Z` at BUILD time only), conformance-against-image instructions (`GATEWAY_CONFORMANCE_URL`).
- Also add: `app.py` gains `def build() -> FastAPI` factory reading `os.environ` (tested via env-injected unit test).

- [ ] **Step 1: Write failing tests** — Dockerfile policy: contains `USER 65532`, does NOT contain `pip install` in the FINAL stage, no `ADD`, pinned base image tag; `build()` factory returns an app when a valid env is set and raises `ConfigError` (safe messages) when not.
- [ ] **Step 2–4: fail → implement → pass.** Manual smoke (best-effort, requires docker): `docker build gateway/ -t elspeth-llm-gateway:dev` — if the daemon is unavailable in this environment, record that in the task report; image build verification then belongs to Phase 5 acceptance.
