...
- [ ] **Step 5b: RC-4 fold — reclassify `examples/row_union_ab_experiment/settings_screened.yaml` (same-slice migration, protocols RC-4).** Rule 4 rejects it (the `quality_screen` gate inside the control branch routes `'false'` to the `screened_out` sink — a path from opener to sink before the closer) and there is NO mechanical migration: its pedagogical point is the prohibited shape. Re-record Task 1's corpus via the documented env var (`ELSPETH_CANONICAL_CORPUS_RECORD=1 pytest tests/unit/core/dag/test_canonical_hash_corpus.py -x`, then assert-mode green): its entry moves from `hashes` to `unbuildable: GraphValidationError` — the roster diff IS the adjudication record. Verify the re-recorded JSON differs from the committed one in EXACTLY that one file; any second mover is an unintended break — STOP and fix before committing. The replacement story ((a) screen-before-fork vs (b) screen-as-loss through the settlement channel) is a maintainer pedagogy call that is STILL OPEN (see Open Questions; protocols RC-4): if the maintainer has ruled by execution time, implement the chosen redesign in THIS commit and update `examples/AGENTS.md` run notes; otherwise land only the reclassification and keep the question open.
...
1. **`examples/row_union_ab_experiment/settings_screened.yaml` replacement story — STILL OPEN (maintainer pedagogy call; 2026-08-22 synthesis kept it open deliberately).** Rule 4 rejects its entire pedagogical point (a mid-branch screen to a sink); Task 7 Step 5b records it unbuildable in the same commit as the rejection. The replacement is the maintainer's choice between protocols RC-4's candidates: (a) screen BEFORE the fork (simplest; loses mid-branch screening pedagogy) or (b) screen-as-loss — route the screened rows via the settlement channel so the union roster settles the member as lost (rule 9 makes the closer a legal in-region `on_error` target). Do not resolve unilaterally.
