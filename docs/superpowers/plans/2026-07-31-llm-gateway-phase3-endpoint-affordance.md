...
- [ ] **Step 2: State the risk transfer, unhedged.** Operator-facing docs must say, in substance: **ELSPETH cannot validate what you put behind that endpoint. It trusts the link you configure. If the endpoint misbehaves — wrong model, silently truncated output, a translation layer that drops fields, a compromised or misconfigured proxy — ELSPETH will faithfully record and act on whatever it returns. Garbage in, garbage out. Owning that endpoint's correctness, availability, and trustworthiness is the operator's responsibility, not ELSPETH's.** Name the concrete failure modes rather than gesturing at them: a proxy that substitutes a cheaper model; one that truncates or reorders tool calls; one that returns a well-formed response with fabricated content; one that logs prompts you believed were private. Note which of these ELSPETH *can* detect (returned model identity is checked and never substituted; usage is never invented; finish reasons are not silently normalised) and which it *cannot* (content fidelity, upstream retention, silent quality degradation).
