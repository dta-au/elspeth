...
- The epoch-57 image (session epoch 57, Landscape epoch 41) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 57
  and Landscape epoch 41.
...
> require both schema checks to pass after initialization at session epoch 57
> and Landscape epoch 41. Record the execution names. Any no-schema dry run
