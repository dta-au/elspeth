...
- The epoch-65 image (session epoch 65, Landscape epoch 43) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 65
...
> require both schema checks to pass after initialization at session epoch 65
