...
- The epoch-56 image (session epoch 56, Landscape epoch 40) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 56
  and Landscape epoch 40.
...
> require both schema checks to pass after initialization at session epoch 56
> and Landscape epoch 40. Record the execution names. Any no-schema dry run
