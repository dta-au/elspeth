...
- The epoch-58 image (session epoch 58, Landscape epoch 42) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 58
...
> require both schema checks to pass after initialization at session epoch 58
