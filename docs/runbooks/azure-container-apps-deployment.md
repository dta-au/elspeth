...
- The epoch-62 image (session epoch 62, Landscape epoch 42) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 62
  and Landscape epoch 42.
...
> require both schema checks to pass after initialization at session epoch 62
> and Landscape epoch 42. Record the execution names. Any no-schema dry run
