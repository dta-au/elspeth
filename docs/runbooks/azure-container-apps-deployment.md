...
- The epoch-67 image (session epoch 67, Landscape epoch 46) in the registry.
...
- `doctor-schema-init` runs `elspeth doctor deployment --init-schema --json`
  with the schema-owner URLs and initializes both schemas at session epoch 67
  and Landscape epoch 46.
...
> require both schema checks to pass after initialization at session epoch 67
> and Landscape epoch 46. Record the execution names. Any no-schema dry run
