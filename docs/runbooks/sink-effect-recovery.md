...
applies to Landscape schema epoch 42 and the `sink-effect-v1` protocol. A
...
groundwork tables, and any store below epoch 35 predates the unified-lineage
flip (tri-column lineage retirement — `token_lineage_frames`/`lineage_path_json`
...
`scheduler_events.seq` replay key that orders scheduler transitions. Any store
below epoch 39 also lacks immutable web run-start permit binding and recoverable
pre-effect admission state. Epoch 40 adds nullable call token measures and
quota-policy/secret-wiring admission evidence. Epoch 41 replaces the fallback
prompt digest with the approved prompt artifact anchor. All are
pre-1.0 recreate
