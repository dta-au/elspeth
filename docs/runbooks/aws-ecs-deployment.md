...
```json
{
  ...
  "schema_facts": {
    "candidate": {"session_epoch": 67, "landscape_epoch": 45, "run_web_plugin_policy_present": true},
    "previous": {"session_epoch": 35, "landscape_epoch": 29, "run_web_plugin_policy_present": true},
    "structural_changes": "session_epoch_35_to_67_landscape_epoch_29_to_45_blob_cleanup_guided_decline_row_union_barrier_and_coordination_schema",
...
session epoch 67, Landscape epoch 45 and `run_web_plugin_policy` presence,
...
not epoch 66. Pre-1.0 candidates do not migrate predecessor schemas: the old
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 45
