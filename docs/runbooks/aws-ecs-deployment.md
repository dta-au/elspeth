...
```json
{
  ...
  "schema_facts": {
    "candidate": {"session_epoch": 62, "landscape_epoch": 42, "run_web_plugin_policy_present": true},
    "previous": {"session_epoch": 35, "landscape_epoch": 29, "run_web_plugin_policy_present": true},
    "structural_changes": "session_epoch_35_to_62_landscape_epoch_29_to_42_blob_cleanup_guided_decline_row_union_barrier_and_coordination_schema",
...
session epoch 62, Landscape epoch 42 and `run_web_plugin_policy` presence,
...
not epoch 62. Pre-1.0 candidates do not migrate predecessor schemas: the old
...
the candidate has recreated Landscape at epoch 42, the 0.7.1 image must
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 42
