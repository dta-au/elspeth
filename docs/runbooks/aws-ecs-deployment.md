...
```json
{
  ...
  "schema_facts": {
    "candidate": {"session_epoch": 47, "landscape_epoch": 31, "run_web_plugin_policy_present": true},
    "previous": {"session_epoch": 35, "landscape_epoch": 29, "run_web_plugin_policy_present": true},
    "structural_changes": "session_epoch_35_to_47_landscape_epoch_29_to_31_blob_cleanup_guided_decline_row_union_barrier_and_scheduler_status_closure",
...
session epoch 47, Landscape epoch 31 and `run_web_plugin_policy` presence,
...
not epoch 47. Pre-1.0 candidates do not migrate predecessor schemas: the old
...
the candidate has recreated Landscape at epoch 31, the 0.7.0 image must
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 31
...
drained and repair forward with epoch-47 session/epoch-31 Landscape code.
