...
session epoch 55, Landscape epoch 40 and `run_web_plugin_policy` presence,
...
not epoch 55. Pre-1.0 candidates do not migrate predecessor schemas: the old
...
the candidate has recreated Landscape at epoch 40, the 0.7.1 image must
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 40
