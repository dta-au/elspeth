...
```json
{
  ...
  "schema_facts": {
    "candidate": {"session_epoch": 57, "landscape_epoch": 41, "run_web_plugin_policy_present": true},
    "previous": {"session_epoch": 35, "landscape_epoch": 29, "run_web_plugin_policy_present": true},
    "structural_changes": "session_epoch_35_to_57_landscape_epoch_29_to_41_blob_cleanup_guided_decline_row_union_barrier_and_coordination_schema",
...
session epoch 57, Landscape epoch 41 and `run_web_plugin_policy` presence,
...
not epoch 57. Pre-1.0 candidates do not migrate predecessor schemas: the old
...
the candidate has recreated Landscape at epoch 41, the 0.7.1 image must
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 41
