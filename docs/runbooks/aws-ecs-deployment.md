...
```json
{
  ...
  "schema_facts": {
    "candidate": {"session_epoch": 63, "landscape_epoch": 43, "run_web_plugin_policy_present": true},
    "previous": {"session_epoch": 35, "landscape_epoch": 29, "run_web_plugin_policy_present": true},
    "structural_changes": "session_epoch_35_to_63_landscape_epoch_29_to_43_blob_cleanup_guided_decline_row_union_barrier_and_coordination_schema",
...
session epoch 63, Landscape epoch 43 and `run_web_plugin_policy` presence,
...
if test "$DEPLOYMENT_MODE" = upgrade; then
  ...
  jq -e '
    ...
    and .schema_facts.candidate.landscape_epoch == 43
