...
## Current Cutover: 0.8.1 replica recovery, identity admission and prompt provenance (session epoch 58 and Landscape epoch 42)

0.8.1 advances `SESSION_SCHEMA_EPOCH` from 53 to 58 and Landscape
...
session epoch is a recreate boundary, including epoch 58. Landscape epoch 42
...
unsupported: keep the service drained, repair the epoch-58 release forward,
recreate fresh state, and retry. The release acceptance record must cite the
session-epoch-58/Landscape-epoch-42 record when binding candidate and rollback
decisions.
...
```bash
sqlite3 "$DB_PATH" 'PRAGMA user_version;'         # expect 58 (== SESSION_SCHEMA_EPOCH)
