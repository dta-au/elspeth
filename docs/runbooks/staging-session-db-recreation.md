...
## Current Cutover: 0.8.1 replica recovery, identity admission and prompt provenance (session epoch 63 and Landscape epoch 43)

0.8.1 advances `SESSION_SCHEMA_EPOCH` from 53 to 63 and Landscape
...
unsupported: keep the service drained, repair the epoch-63 release forward,
recreate fresh state, and retry. The release acceptance record must cite the
session-epoch-63/Landscape-epoch-43 record when binding candidate and rollback
decisions.
...
```bash
sqlite3 "$DB_PATH" 'PRAGMA user_version;'         # expect 63 (== SESSION_SCHEMA_EPOCH)
