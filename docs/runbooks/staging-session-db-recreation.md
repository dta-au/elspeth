...
## Current Cutover: 0.8.1 replica recovery, identity admission, prompt provenance and call mode audit (session epoch 67 and Landscape epoch 45)

0.8.1 advances `SESSION_SCHEMA_EPOCH` from 53 to 67 and Landscape
`SQLITE_SCHEMA_EPOCH` from 38 to 45. Session epoch 54 adds durable Composer
...
session epoch is a recreate boundary, including epoch 62. Landscape epoch 45
is the current release boundary, so a Landscape database left at epoch 44 or
...
session-epoch-67/Landscape-epoch-45 record when binding candidate and rollback
decisions.
...
sqlite3 "$LANDSCAPE_PATH" 'PRAGMA user_version;'  # expect 45 (== SQLITE_SCHEMA_EPOCH)
```
