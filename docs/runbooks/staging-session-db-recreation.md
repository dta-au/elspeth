...
## Current Cutover: 0.7.2 blob cleanup, guided decline and replay, coordination, and row_union barrier with diagnostics attribution (session epoch 44 and Landscape epoch 30)

0.7.2 advances `SESSION_SCHEMA_EPOCH` from 35 to 44. Epoch 36 ensures a committed
...
attributed to their real writer instead of the compose loop. Epoch 44 adds persistent
session-operation authority, compatible-generation membership and run-start
state, durable ticket/composer/rate-limit handoff, bounded cleanup claims,
monotonic user-secret row versions, and proposal blob-effect receipts.
...
`run_diagnostics` chat writer principal (43). Compatible-generation
membership, persistent session-operation authority, cross-replica handoff,
bounded cleanup claims, and proposal blob-effect receipts advance the session
store to epoch 44. The
...
```bash
sqlite3 "$DB_PATH" 'PRAGMA user_version;'  # expect 44 (== SESSION_SCHEMA_EPOCH)
```

An epoch-35 through epoch-43 result is not repairable in place: keep the
