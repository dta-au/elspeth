...
## Current Cutover: 0.8.1 replica recovery, identity admission, prompt provenance and call mode audit (session epoch 68 and Landscape epoch 46)

0.8.1 advances `SESSION_SCHEMA_EPOCH` from 53 to 68 and Landscape
...
session epoch is a recreate boundary, including epoch 62. Landscape epoch 46
is the current release boundary, so a Landscape database left at epoch 45 or
...
unsupported: keep the service drained, repair the epoch-68 release forward,
recreate fresh state, and retry. The release acceptance record must cite the
session-epoch-68/Landscape-epoch-46 record when binding candidate and rollback
decisions.
...
```bash
sqlite3 "$DB_PATH" 'PRAGMA user_version;'         # expect 68 (== SESSION_SCHEMA_EPOCH)
