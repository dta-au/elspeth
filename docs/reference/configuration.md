...
`group_losses`) and `token_work_items.lineage_path_json`.
See the
...
34, either automatically at startup or through an operator migration command.
...
an older epoch must not be rolled back over an epoch-34 database.
