...
| `DIVERT` | Token diverted to error/quarantine sink | Source `on_validation_failure`, transform `on_error`, config-gate `on_error`, aggregation `on_error` |
...
| Error divert | `"__error_{producer_name}__"` | transform_2, quality_gate or batch_stats aggregation → error_sink |
...
- Optional named `on_error` policy creates a structural DIVERT edge
...
- Config-gate `on_error` → error sink (if configured and not `"discard"`)
- Aggregation `on_error` → error sink (if not `"discard"`). An unknown sink is
  a `GraphValidationError` here. Unlike transforms and gates there is no rule-9
  closer target: rule 6 keeps aggregations out of every bound region, so a
  closer name is an unknown sink (elspeth-d2e3f29d10)
- Label format: `"__error_{producer_name}__"` or `"__quarantine__"`
...
After schema validation, `from_plugin_instances()` calls `warn_divert_coalesce_interactions()` to detect a dangerous pattern: if a transform or intermediate gate between a fork gate and a `require_all` coalesce has `on_error` routing (a DIVERT edge), any row hitting that error path will cause the coalesce to permanently lose a branch. The originating fork gate is the scan boundary: its own `on_error` does not strand a group because the row never forked.
...
The same build pass calls `warn_divert_row_union_interactions()` for the sibling correlated barrier: if a transform or intermediate gate in a branch chain feeding a `row_union` has `on_error` routing (a DIVERT edge), a diverted row never arrives at the barrier. As with coalesce, the originating fork gate itself is excluded because its diverted row never creates a correlated group.
...
These produce `GraphValidationWarning` objects (non-fatal) with code `DIVERT_ROW_UNION_GROUP_LOSS`, using the same shape shown above; `node_ids` carries the row_union node followed by the diverting processing nodes. The warnings are logged via structlog. Only `MOVE` incoming edges are inspected — identity branches (a `COPY` straight from the fork gate to the barrier) have no intermediate processing nodes, so nothing in-branch can divert.
...
gates:
  - name: amount_threshold
    ...
    routes:
      "true": high_values
      "false": standard
    on_error: gate_errors    # Optional sink name or "discard"
```

When a valid gate condition fails while evaluating one row:

1. The gate node state is completed as `FAILED` with structured error evidence.
2. A named `on_error` sink routes the row through the gate's structural DIVERT
   edge and records `(failure, on_error_routed)`.
3. `on_error: discard` records `(failure, gate_error_discarded)` with an
   `error_hash`; no sink or routing event is fabricated.
4. If `on_error` is omitted, the evaluation error propagates and execution
   remains fail-fast.
5. A configured policy affects only the bad row; processing continues for
   unaffected source rows.

Syntax and expression-security failures are config errors and never enter this
row-error path.
...
- They use label format `"__error_{producer_name}__"` or `"__quarantine__"`
