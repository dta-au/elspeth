...
| `(failure, on_error_routed)` | Transform processing or config-gate expression evaluation failed and `on_error` routed the token to an error sink; or an aggregation batch failed and its `on_error` routed every buffered token (with its original row) to the named sink. | Transform and gate executors plus error handling in `src/elspeth/engine/token_traversal.py`; the failed-flush arm of `RowProcessor.handle_timeout_flush` (`_route_flush_error_to_sink`). Recorded by the sink after durability. |
