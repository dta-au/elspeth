...
| **barrier scope** | The SESE region of any *bound* group — between its opener and its closer. The fork→coalesce region always was one; rev 3 makes that literal. **Never "span":** `engine/spans.py` is the OpenTelemetry span factory (`SpanFactory`, `run_span`, `span_id`, exported on the wire to OTLP collectors), and an engine span is *also* a bounded, nested region of pipeline execution carrying run/token correlation — the collision would be exact. The distinction the two names must keep visible: an engine span **observes** execution and has no control semantics; a barrier scope **governs** it and can hold or fail the group. Implementation/validation code says **bound region** for the same concept (`core/dag/bound_regions.py`, `BoundRegion`, `max_bound_region_depth`) |
