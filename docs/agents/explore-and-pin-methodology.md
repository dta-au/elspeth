...
Status: method, first executed on elspeth-68721c71d7 (planner repair-feedback
teaching, merged into release/0.8.0 at 51435dbb5, 2026-09-02). Written so the
next seam gets the same treatment without re-deriving the process.
...
| Freeform tool-result envelope | `ToolResult` fields in `tools/_common.py` | redaction manifest in `redaction.py` (fails closed with an unknown-response placeholder) | composer skill and catalogue prose | nominated |
...
## 17. Effort shape of the first run

For sizing a nomination "for the same effort":

| Item | First seam |
|---|---|
| Distinct keys adjudicated | 64 (63 teach, 1 fence) |
| Shipped rows in census | 245 |
| Commits on the branch | 11 |
| Reviewer seats | 3 |
| Red-team rounds to zero | 5 |
| Findings fixed | 1 major, 5 minor |
| Live scenarios | 3 battery + the seam scenario |

## 18. Traps met on the first run
