...
- **2026-08-26 — the guided deferral surface is PLURAL end to end: one resolution plus 1..K retains per reply** (elspeth-3a21f09f09)
  ...
  5. The repair thread answers EVERY call id with per-call errors (`rejected_calls=`/`errors=` aligned tuples), but protocol completeness is not action custody: parsed-valid siblings live in ordered repair slots across the retry. A retry that resends only rejected calls fills those slots; a retry that obeys "resend ALL" is authoritative as a complete group and must not duplicate the first pass. Never equality-deduplicate actions — two identical calls still mean two actions.
  6. Resolution repair has the same custody rule: valid grouped retains survive a corrected-only resolution reply, or return alone with a withheld-resolution outcome when the retry declines in prose. Step-3/wire chats never offer `retain_deferred_intent`, so there is deliberately no third solver site to sweep.
  See [CONTRIBUTING: Convention: web composer and frontend](../../CONTRIBUTING.md#convention-web-composer-and-frontend).
