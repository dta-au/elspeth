...
- **2026-08-26 — the guided deferral surface is PLURAL end to end: one resolution plus 1..K retains per reply** (elspeth-3a21f09f09)
  ...
  5. The repair thread answers EVERY call id with per-call errors (`rejected_calls=`/`errors=` aligned tuples), but protocol completeness is not action custody: parsed-valid siblings live in ordered repair slots across the retry, and an admitted current-stage resolution sibling is held in an owned carrier. A retry that resends only rejected calls fills those slots and restores the held resolution; a complete replay is authoritative and must not duplicate the first pass. A reply with no valid retain cohort (prose, empty, resolution-only, management, or hallucinated tools) does NOT replace the transaction: re-raise the owned first shape error so the caller durably retains the whole original Send for clarification. Never equality-deduplicate actions — two identical calls still mean two actions.
  6. Resolution repair has the same custody rule in the other direction: valid grouped retains survive a corrected-only resolution reply, or return alone with a withheld-resolution outcome when the retry declines in prose. Step-3/wire chats never offer `retain_deferred_intent`, so there is deliberately no third solver site to sweep.
  See [CONTRIBUTING: Convention: web composer and frontend](../../CONTRIBUTING.md#convention-web-composer-and-frontend).
