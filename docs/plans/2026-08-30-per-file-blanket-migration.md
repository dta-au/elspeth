...
- [ ] **Step 7: Stage one bundle** (key-free shell — `stage_scan` fails closed if the HMAC key is in the environment): `mcp__elspeth-judge__stage_scan` → bundle id; `stage_annotate` with the merged sidecars for the wave's justify keys (join on exact `ast=` path first, fingerprint second, unique `(file, rule, symbol)` triple third — never fuzzy); `stage_preview` to surface BLOCKs; `stage_status` for the paste-ready command. Expected: `new_judgment` action count == number of uncovered findings after Step 3; un-annotated count 0.
