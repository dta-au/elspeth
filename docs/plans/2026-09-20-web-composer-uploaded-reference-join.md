...
2. Upload a UTF-8 text file through the authenticated upload API, have the actual Composer tool loop wire its blob marker to an LLM `prompt_template` or query template, validate, approve, and run against a deterministic provider stub. Assert the exact prompt bytes received by the provider and the approved prompt artifact hash/inline-resolution audit agree. Include session mismatch, tampered bytes, and LLM-authored blob refusal. This tests the complete authorization and attestation seam without paying or depending on a live provider.
