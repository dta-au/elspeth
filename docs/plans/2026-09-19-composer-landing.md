...
- Phase 1 bug `elspeth-cb0d4b8dba` is still `fixing` with an expired claim.
  Phase 2 task `elspeth-b0ef01ea25` is open and blocked by Phase 1. Reconcile
  claim ownership before changing either ticket.
...
1. Review the panel on the rebased candidate in freeform and guided sessions,
   ...
   pointers, keyboard focus, live announcements and a narrow viewport. Make
   `Open checks` navigate to the visible Pipeline Checks view on narrow screens
   before calling that affordance complete. Keep the documented reload loss of
