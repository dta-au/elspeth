...
2. Replace the Freeform header's direct Guided button with a clearly named secondary “Composer options” menu containing “Switch to guided.” Keep authority information visible. Reuse the existing goal-entry/confirmation behaviour rather than creating a second conversion path. Opening the menu alone must make no mode-changing/provider request.
