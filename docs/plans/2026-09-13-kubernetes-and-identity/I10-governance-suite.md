...
- Consumes:
  ...
  - I9: routes `GET /api/workflow/mailbox/summary` → `MailboxSummaryResponse(governance, roles, approvals_to_decide, reviews_to_attest, decisions_unseen)`, `GET /api/workflow/mailbox/inbox` → `{approvals: [ApprovalView], reviews: [ReviewRequestView]}`, `GET /api/workflow/mailbox/sent` → `{approvals: [ApprovalView]}`, `POST /api/workflow/mailbox/{approval_id}/seen` → `ApprovalView`; `create_mailbox_router()` registered in `web/app.py` (I9 Step 15); tests `test_sent_and_seen_round_trip_clears_the_unseen_count`, `test_summary_counts_each_folder_and_reports_live_roles` in `tests/unit/web/workflow/test_mailbox_routes.py`; PostgreSQL file `tests/testcontainer/web/test_workflow_mailbox_postgres.py`.
...
GOVERNANCE_SUITE: Final[Sequence[GovernanceItem]] = (
    ...
    GovernanceItem(
        ...
        fire=(
            ...
            Pin(_APPROVAL_ROUTES, "test_any_live_approver_decides_once_and_the_second_decider_gets_the_current_state"),
            Pin(_SUITE, "test_round_trip_request_decide_see_and_the_badge_clears"),
        ),
    ...
    GovernanceItem(
        ...
        mutation=(
            Pin(_APPROVAL_AUTHORITY, "test_any_active_approver_who_is_not_the_author_may_decide"),
...
def test_round_trip_request_decide_see_and_the_badge_clears(governed: _Governed) -> None:
    ...
    # The losing half of the decide guard: a second live approver arrives after the decision.
    late = client.post(f"/api/approvals/{approval_id}/decide", headers=carol, json={"decision": "approved", "note": None})
    ...
    # R4: both rows were written before the responses; carol's refused decide wrote none.
