...
- Produces, all in `docs/plans/2026-09-13-kubernetes-platform-facts.md` and copied verbatim by later tasks:
  ...
  - §2.5 wall times: K0 records `kind create`, `docker build`, `kind load`, `kind delete` on this box; the `[K4]` row and §5.1 are written by Task K4 Step 6 from two consecutive development-box runs of `scripts/cicd/kubernetes-kind-smoke.sh` (cold, warm), checked against the `kubernetes-kind` job's `timeout-minutes: 60` budget (DECISIONS K9).
