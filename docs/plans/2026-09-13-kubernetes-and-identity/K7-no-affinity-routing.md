...
def _apply_and_roll(kind_cluster: KindCluster, overlay: Path, *, settle_timeout: float = 300.0) -> None:
    """Apply an overlay, force a fresh rollout, and wait until the Service answers.
...
def test_composer_turn_in_flight_on_one_replica_is_visible_from_the_other(no_affinity_rollout: KindCluster) -> None:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        ...
        # In-flight poll through B (DECISIONS K15): the stalled sidecar holds A's
...
Expected: `exit=1`. `test_client_ip_affinity_pins_one_replica_per_client` PASSES (the control runs on K4's overlay, which exists). The other three ERROR at fixture setup with the `AssertionError` that `KindCluster.kubectl` raises on a non-zero exit — `kubectl apply -k <repo>/tests/testcontainer/deployment/kubernetes/no-affinity-overlay failed (exit=1):` followed by kubectl's stderr, which carries `error: must build at directory: not a valid directory` for the missing overlay. A `NameError` or an import-time error that also takes down K4's three proofs means Step 1's import block or a K4 name was not reused as written. A control that FAILS here (scatter set of size 2 under ClientIP) means the scatter instrument or K4's Service is wrong; stop and fix that before writing the overlay.
...
Prove the render before touching the cluster (structural, no API server — DECISIONS K6):
...
In `deploy/kubernetes/overlays/aks/ingress.yaml` (K6's file — DECISIONS K19), comment out ONLY the eight cookie-affinity annotations and rewrite the header comment that says affinity stays on until K7. The file from its first line through the annotation block, before the edit, as K6 writes it:
