...
- The required `gateway_*` inputs (image digest + revision + repository, three
  ...
  model-mappings JSON covering both composer models, OAuth client authentication
  method, and five positive request bounds). The example tfvars supplies
  reviewable values for the method and bounds; set them for the agency upstream.
