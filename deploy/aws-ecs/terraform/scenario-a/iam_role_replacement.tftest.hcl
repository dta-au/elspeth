...
variables {
  ...
  purpose                         = "IAM role replacement contract"
...
run "replace_iam_role_contract" {
  command = plan

  variables {
    aws_account_id = join("", ["210987", "654321"])
    candidate_image = format(
      "%s.dkr.ecr.ap-southeast-1.amazonaws.com/elspeth-web@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      join("", ["210987", "654321"]),
    )
    iam_permissions_boundary_arn = format(
      "arn:aws:iam::%s:policy/elspeth-%s-ecs-boundary",
    ...
    cloudwatch_agent_image = format(
      "%s.dkr.ecr.ap-southeast-1.amazonaws.com/elspeth-cloudwatch-agent@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
      join("", ["210987", "654321"]),
    )
    bedrock_inference_profile_arns = [
      format("arn:aws:bedrock:ap-southeast-1:%s:inference-profile/apac.primary-profile", join("", ["210987", "654321"])),
      format("arn:aws:bedrock:ap-southeast-1:%s:inference-profile/apac.advisor-profile", join("", ["210987", "654321"])),
    ]
  }
