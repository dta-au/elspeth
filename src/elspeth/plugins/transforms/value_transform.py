...
class ValueTransform(BaseTransform):
    ...
    source_file_hash: str | None = "sha256:5ea7088876ccbcc2"
        ...
        if issue_code is None:
            return PluginAssistance(
                ...
                composer_hints=(
                    ...
                    "overwrites, string concatenation (row['a'] + '-suffix'), case folding and whitespace trimming in "
                    "function-call form (lower(row['x']), upper(row['x']), strip(row['x']), casefold(row['x'])), membership "
                    "tests, and conditional values (1 if row['score'] > 10 else 0). For type changes use type_coerce.",
                    "NOT achievable here: regular expressions (pattern extraction, splitting), title-casing, and method-call "
                    "syntax (row['x'].lower() is rejected — use lower(row['x'])) — only len(), abs(), lower(), upper(), "
                    "strip(), and casefold() are callable. Route text rewriting beyond case folding through an llm "
                    "transform instead.",
