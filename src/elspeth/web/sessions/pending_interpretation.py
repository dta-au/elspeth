...
class _SessionPendingInterpretationValidator:
    ...
    def __call__(
        ...
        data = candidate.data
        candidate_state = state_from_record(
            replace(
        ...
        # verdict (elspeth-155947ca47): when the authoring gate passes and a
        # runtime preflight is wired, its verdict is merged in so an
...
class _SessionPendingInterpretationPlanner:
    ...
    def plan(
        ...
        if kind is InterpretationKind.VAGUE_TERM:
            ...
            final_sources, final_nodes, resolved_hash = _resolve_source_data_contract(
                live_state,
                event_id=str(event_id),
                affected_node_id=affected_node_id,
                user_term=user_term,
