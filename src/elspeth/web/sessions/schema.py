...
_EPOCH_42_COORDINATION_EXPIRY_INDEXES: dict[str, str] = {
...
_EPOCH_42_COORDINATION_TABLES: frozenset[str] = frozenset({*_EPOCH_42_COORDINATION_EXPIRY_INDEXES, "run_execution_inputs"})
...
def _validate_current_schema(bind: Engine | Connection) -> None:
    ...
    _validate_epoch_42_coordination_metadata()
...
def _validate_epoch_42_coordination_metadata() -> None:
    ...
    from the declared metadata. Epoch 42 names this exact coordination
    substrate, so startup also verifies the model-side release contract.
    """

    if SESSION_SCHEMA_EPOCH != 42:
        _schema_error("coordination schema epoch mismatch", expected=42, actual=SESSION_SCHEMA_EPOCH)
    missing_tables = _EPOCH_42_COORDINATION_TABLES - set(metadata.tables)
    if missing_tables:
        _schema_error(
            "epoch-42 coordination table set mismatch",
            expected=sorted(_EPOCH_42_COORDINATION_TABLES),
            actual=sorted(_EPOCH_42_COORDINATION_TABLES - missing_tables),
        )
    ...
    for table_name, expiry_index_name in _EPOCH_42_COORDINATION_EXPIRY_INDEXES.items():
