...
| `aggregation_numeric_value_field_type_mismatch_against_source_schema` | The aggregation's numeric `value_field` flows unchanged from an observed CSV source, so it arrives as `str` and the batch is rejected at runtime. Declare the field with a numeric type in the SOURCE schema (the source coerces at ingestion), or insert a `type_coerce` before the aggregation with a `conversions` entry targeting the numeric type. If the field is categorical, use `batch_top_k` instead of a numeric aggregation. |
