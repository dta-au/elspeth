...
| CSV `reference_join` output reaches a numeric consumer | CSV lookup cells remain strings, including numeric-looking values such as `24`; JSON numbers retain numeric types. Preserve downstream `str` contracts when acceptable, or insert `type_coerce` after the join before the numeric consumer. For example, `conversions: [{field: amount, to: int}]` converts a joined `amount` field. Its schema declares the arriving `str`, not the converted `int`. A `field_mapper` or sink schema does not convert values. Never replace supplied CSV with JSON to repair types; typed JSON is an option only when the user's data requirements permit that format. |
