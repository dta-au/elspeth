...
_VALIDATION_ERROR_PATTERNS: Final[tuple[tuple[str, str, str], ...]] = (
    ...
    (
        ...
        "or require_all. Call preview_pipeline until the violation is gone — the first repair above empties the sink's "
        "required_fields, which REMOVES that edge_contracts row rather than turning its satisfied true. A source "
        "guarantee names only "
...
_PREVIEW_PIPELINE_DECLARATION = ToolDeclaration(
    ...
    "checked, and a pair is checked only where the consumer declares "
    "required fields, so an edge declaring none has no entry and an empty "
