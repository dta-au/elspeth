...
function backendWebServerBlock(): string {
  ...
  const match = source.match(
    /command:\s*\n\s*"npm --prefix src\/elspeth\/web\/frontend run build && "[\s\S]*?"uv run --extra webui python -m uvicorn elspeth\.web\.app:create_app --factory "[\s\S]*?env: composerSettingsEnv,\s*}/,
  );
  if (match === null) {
...
describe("playwright backend webServer isolation", () => {
  it("always starts the managed backend with the E2E environment", () => {
    ...
    expect(backend).toContain("npm --prefix src/elspeth/web/frontend run build");
