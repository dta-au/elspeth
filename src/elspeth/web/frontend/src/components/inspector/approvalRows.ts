...
export interface ApprovalRow {
  ...
  plugin: string | null;
  /** The step's configuration NOW; `value` is what was approved THEN. */
  binding: string | null;
  value: string;
  resolvedAt: string;
  dateLabel: string;
...
/** One row per approved interpretation, shared by the Workflow tab's Approvals
 *  table and the Approvals tab. Throws on a malformed approval (fail closed);
 *  both callers sit inside an ErrorBoundary. */
export function approvalRows(
  ...
  return events.map((event) => {
    ...
    return {
      id: event.id,
      name,
      nodeName: source?.[0] ?? event.affected_node_id,
      plugin: component?.plugin ?? null,
      binding: component ? pluginBindingLabel(component.plugin, component.options) : null,
      value: event.accepted_value,
      resolvedAt: event.resolved_at,
      dateLabel: Number.isNaN(approvedAt.getTime())
        ? event.resolved_at
