...
const READ_ONLY_TOOL_CALL_DESCRIPTIONS: Record<string, string> = {
  ...
  get_blob_content: "Reads the text content of a session file for inspection.",
  get_blob_metadata: "Looks up a session file's name, size, and status.",
  inspect_source:
    "Reads a session file's headers, inferred types, and sample-row count to plan the source.",
...
const MUTATING_TOOL_CALL_DESCRIPTIONS: Record<string, string> = {
  ...
  patch_node_options:
    "Updates configuration options on a pipeline node.",
  ...
  delete_blob: "Deletes a session file and its storage.",
  update_blob: "Replaces the content of a session file.",
  set_source_from_blob: "Wires a session file as the pipeline's data source.",
  set_source_from_blobs:
    "Wires one or more files containing user-provided content as a one-row-per-file data source.",
  splice_transform: "Inserts a transform between two connected steps.",
  wire_blob_inline_ref:
    "Pins a session file's content into a configuration field by hash.",
  wire_secret_ref:
