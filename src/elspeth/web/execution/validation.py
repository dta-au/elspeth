...
class _GateFanOutFinding:
    """One gate that delivers every row to the same set of terminal destinations.

    Attributes:
        node_id: ID of the gate.
        condition: The gate's authored condition, verbatim — the author needs
            to see that it reads no row data.
        destinations: The shared destination set, sorted. Always two or more;
            a single shared destination duplicates nothing.
    """
...
def _find_gate_fan_out_advisories(state: CompositionState) -> list[_GateFanOutFinding]:
    """Detect constant-condition gates that write every row to every destination.

    A gate is flagged iff ALL of the following hold:

    1. ``node_type == "gate"`` with no plugin (a config gate).
    2. The condition is a bare literal — it reads no row data, so every row
       takes the same branch (:func:`gate_condition_is_constant`).
    3. The gate is boolean-routed: route labels are exactly ``{"true","false"}``.
    4. Both routes resolve — through :func:`gate_route_destinations`, the one
       authority on the reserved ``fork``/``discard`` keywords — to the SAME
       destination set, of two or more destinations.
    5. Every destination is undifferentiated and terminal: no node consumes it
       as an input, and no coalesce/row_union names it as a branch. A branch
       carrying its own per-branch work, or rejoining a barrier, is the
       documented fan-out macro and is left alone.

    This is ADVISORY and never blocks. The shape is legal, is what
    ``pipeline_composer.md`` teaches for dual independent outputs, and is
    generated by ELSPETH's own authoring aids — but it is also exactly what an
    author gets when a stated routing condition never reached the gate
    (acceptance run 2, R2-F17), and the two are indistinguishable from the
    graph alone. So the advisory names the consequence and both remedies and
...
def validate_pipeline(
    ...
    *,
    ...
    # Gate fan-out advisory — same non-blocking, happy-path-only contract as
    # the identity advisory above. The shape is legal (it is the documented
    # fan-out macro), so this must never gate a run; it exists because the
    # identical shape is what a dropped routing condition produces
    # (acceptance run 2, R2-F17 / elspeth-5c0c09db31), and only the author can
    # tell the two apart.
    for fan_out_finding in _find_gate_fan_out_advisories(state):
        destination_text = ", ".join(f"'{destination}'" for destination in fan_out_finding.destinations)
        checks.append(
            ValidationCheck(
                name=_CHECK_GATE_FAN_OUT_ADVISORY,
                passed=True,
                detail=(
                    f"Gate '{fan_out_finding.node_id}' has the constant condition "
                    f"{fan_out_finding.condition!r}, so it makes no per-row decision, and both of its "
                    f"routes deliver to the same destinations ({destination_text}). Every row will be "
                    f"written to all {len(fan_out_finding.destinations)} of them. If that fan-out is "
                    "intended, nothing needs to change. If each row was meant to land in exactly one "
                    "destination, replace the condition with the rule that decides between them "
                    "(for example \"row['amount'] > 500\") and point the 'true' and 'false' routes at "
    ...
    # (generating rows) mis-declared as a transform — so this never
    # escalates to a rejection, now or later. Wording deliberately asks
    # rather than asserts ("did you mean...?") because an LLM source plugin
    # is in development elsewhere but does not exist yet: naming a specific
    # plugin id here would promise a capability that may not be installed or
    # allowlisted on a given deployment even once it lands. Revisit this
    # string once that plugin ships and its id is verifiable.
