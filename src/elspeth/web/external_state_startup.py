"""Provider-neutral external PostgreSQL startup contract."""
...
_DOCTOR_GUIDANCE = "Run 'elspeth doctor deployment' for full diagnostics."


class ExternalStateStartupContractError(RuntimeError):
    """Raised when the external-state startup contract is not satisfied."""


class ExternalStateSchemaNotReadyError(RuntimeError):
    """Raised when an external database cannot prove its current schema."""


def _contract_error(detail: str) -> ExternalStateStartupContractError:
    return ExternalStateStartupContractError(f"{detail} {_DOCTOR_GUIDANCE}")


def _schema_error(label: str) -> ExternalStateSchemaNotReadyError:
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            now = monotonic()
            _slog.warning(
                "external_state_schema_probe_retry",
                label=label,
                attempt=attempt,
